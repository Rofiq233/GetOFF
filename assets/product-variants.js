...
class ProductVariants extends HTMLElement {
    ...
    connectedCallback() {
        ...
        this.updateAvailableOptions(this.selectedOptions());
    ...
    updateAvailableOptions = (selectedOption) => {
        ...
        this.querySelectorAll(".product-variants__option").forEach((optionElement) => {
            ...
            optionElement.querySelectorAll(".product-variants__input")
                .forEach((input) => {
                    input.disabled = !availableOptions.includes(input.value);
    ...
    updatePrice = (variant) => {
        this.closest(".sellers__item").querySelector(".sellers__item-price").innerText = Shopify.formatMoney(variant.price, Shopify.money_format);
    }
    updateImage = (variant) => {
        const image = variant.featured_image;
        if (!image) {
        ...
        const EleImage = this.closest(".sellers__item").querySelector("img");
        if (!EleImage) {
            return;
        }
        EleImage.src = image.src;
        const widths = [200, 300, 400, 500];

        EleImage.srcset = widths.map((width) => {
            return `${image.src}&width=${width} ${width}w`;
        }).join(', ');
